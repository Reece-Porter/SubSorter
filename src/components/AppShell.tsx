...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <ReminderManager />
      {/* Desktop / top header */}
      <header className="sticky top-0 z-30 border-b border-hairline bg-paper/80 backdrop-blur">
          ...
          <div className="flex h-16 items-center justify-between">
            <Link href="/" className="flex items-center gap-2.5">
              ...
              <span className="flex flex-col leading-tight">
                <span className="text-[15px] font-semibold tracking-tight text-ink-900">Subscription Ghost</span>
      ...
      <nav className="sm:hidden fixed bottom-0 inset-x-0 z-30 border-t border-hairline bg-card/95 backdrop-blur pb-[env(safe-area-inset-bottom)]">
          ...
          {NAV.map(({ href, label, icon: Icon }) => (
            <Link
              ...
              className={cx(
                ...
                isActive(href) ? "text-ink-900" : "text-ink-500"
              )}
            >
              <Icon width={22} height={22} />
              {label}
              {href === "/review" && flaggedCount > 0 && (
                <span className="absolute right-1/2 top-1.5 translate-x-3.5 rounded-full bg-accent-500 px-1 text-[9px] font-semibold text-white">
                  {flaggedCount}
                </span>
              )}
            </Link>
