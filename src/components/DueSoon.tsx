...
export function DueSoon({ subs }: { subs: Subscription[] }) {
  ...
  return (
    <section className="rounded-2xl border border-accent-100 bg-accent-50/60 p-4 sm:p-5">
      <div className="flex items-center gap-2">
        <span className="grid h-7 w-7 place-items-center rounded-lg bg-accent-500/12 text-accent-ink">
          <IconBell width={16} height={16} />
        </span>
        <h2 className="text-sm font-semibold text-ink-900">Due soon</h2>
        <Badge tone="accent">{dueSoon.length}</Badge>
        <span className="ml-auto text-xs text-ink-500 hidden sm:inline">Renewing within {windowDays} days</span>
      </div>
      <ul className="mt-3 divide-y divide-accent-100/70">
        {dueSoon.map((s) => {
          const d = daysUntil(s.nextBillingDate);
          const u = urgencyFor(s.nextBillingDate);
          const critical = u === "critical" || u === "overdue";
          return (
            <li key={s.id} className="flex items-center justify-between gap-3 py-2">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <span className="tnum text-sm font-medium text-ink-800">{formatMoney(s.cost, settings.currency)}</span>
                <span
                  className={cx(
                    "tnum rounded-full px-2 py-0.5 text-xs font-semibold",
                    critical ? "bg-accent-500 text-white" : "bg-accent-50 text-accent-ink ring-1 ring-inset ring-accent-100"
                  )}
                >
                  {d <= 0 ? "today" : `${d}d`}
                </span>
